#!/usr/bin/env python3
"""
Convert Moorea Privacy Policy from Markdown to PDF using a simpler approach
"""
...
def convert_markdown_to_pdf():
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"✅ PDF created successfully using wkhtmltopdf: {pdf_filename}")
        else:
            raise Exception("wkhtmltopdf failed")
            
    except (FileNotFoundError, Exception):
        # Fallback: create a nicely formatted HTML file that can be printed to PDF
        html_filename = 'Moorea_Privacy_Policy.html'
        with open(html_filename, 'w', encoding='utf-8') as f:
            f.write(html_document)
        
        print(f"⚠️  wkhtmltopdf not available. Created HTML file: {html_filename}")
        print(f"📄 You can open this file in your browser and print to PDF")
        print(f"📍 Location: {os.path.abspath(html_filename)}")
        
        # Also try to use the system's print-to-PDF functionality
        try:
            subprocess.run(['open', '-a', 'Safari', html_filename])
            print(f"🌐 Opened in Safari - you can now print to PDF")
        except:
            pass
    ...
    if os.path.exists(pdf_filename):
        print(f"📄 PDF file size: {os.path.getsize(pdf_filename) / 1024:.1f} KB")
...
if __name__ == "__main__":
    try:
        output_file = convert_markdown_to_pdf()
        print(f"\n🎉 Privacy Policy is ready!")
        print(f"📁 You can find it at: {os.path.abspath(output_file)}")
